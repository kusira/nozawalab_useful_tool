...
from common.constants import RESIZE_METHODS
...
class UIBuildMixin:
    ...
    # 画像処理スライダー（寸法を変えない処理のみ。表示の見た目を調整する）
    PARAM_SLIDERS = [
    ...
    def _build_menu(self) -> None:
        ...
        file_menu.add_cascade(label="処理後画像をエクスポート", menu=export_menu)
        export_menu.add_command(label="PNG", command=lambda: self.export_display("png"))
        export_menu.add_command(label="JPEG", command=lambda: self.export_display("jpeg"))
        export_menu.add_command(label="NPY（解析配列）", command=lambda: self.export_display("npy"))
        export_menu.add_separator()
        export_menu.add_command(label="名前を付けて保存...", command=self.export_display_as)
    ...
    def _build_view_panel(self, parent: ttk.Frame) -> None:
        ...
        transform_row = ttk.Frame(parent)
        transform_row.pack(fill=tk.X, pady=(4, 0))
        ttk.Label(transform_row, text="変換:").pack(side=tk.LEFT)
        ttk.Button(transform_row, text="左右反転", command=self.flip_horizontal).pack(side=tk.LEFT, padx=(4, 0))
        ttk.Button(transform_row, text="上下反転", command=self.flip_vertical).pack(side=tk.LEFT, padx=(4, 0))
        ttk.Button(transform_row, text="90°回転", command=lambda: self.rotate90(-1)).pack(side=tk.LEFT, padx=(4, 0))
        ttk.Button(transform_row, text="-90°回転", command=lambda: self.rotate90(1)).pack(side=tk.LEFT, padx=(4, 0))
    ...
    def _build_analysis_panel(self, parent: ttk.Frame) -> None:
        ...
        tab_adjust = ttk.Frame(notebook, padding=6)
        ...
        notebook.add(tab_adjust, text="調整")
    ...
    def _build_adjust_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(header, text="画像処理（表示の見た目）", font=("", 10, "bold")).pack(side=tk.LEFT)
        ttk.Button(header, text="リセット", command=self.reset_params).pack(side=tk.RIGHT)
        ttk.Label(
            parent,
            text="統計・ヒストグラム等の解析は生データに対して行われます（見た目の調整は解析結果に影響しません）。",
            foreground="#888888",
            wraplength=380,
        ).pack(anchor=tk.W, pady=(2, 4))

        for key, label, vmin, vmax, default in self.PARAM_SLIDERS:
            self._add_param_slider(parent, key, label, vmin, vmax, default)
        ...
        ttk.Scale(
            mag_frame, from_=4, to=100, orient=tk.HORIZONTAL,
            variable=self.magnifier_size, command=self._on_magnifier_radius_change,
        ...
        ttk.Scale(
            mag_frame, from_=1, to=32, orient=tk.HORIZONTAL,
            variable=self.magnifier_zoom, command=self._on_magnifier_zoom_change,
        ).pack(fill=tk.X, pady=(2, 0))

        exp_frame = ttk.LabelFrame(parent, text="処理後画像のエクスポート", padding=6)
        exp_frame.pack(fill=tk.X, pady=(8, 0))
        dir_row = ttk.Frame(exp_frame)
        ...
        self.export_dir_label = ttk.Label(exp_frame, text=str(self.export_dir), wraplength=360, foreground="#555555")
        self.export_dir_label.pack(anchor=tk.W, pady=(4, 6))
        btn_row = ttk.Frame(exp_frame)
        btn_row.pack(fill=tk.X)
        ttk.Button(btn_row, text="PNG", command=lambda: self.export_display("png")).pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 3)
        )
        ttk.Button(btn_row, text="JPEG", command=lambda: self.export_display("jpeg")).pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=(3, 3)
        )
        ttk.Button(btn_row, text="NPY", command=lambda: self.export_display("npy")).pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=(3, 0)
        )
        ttk.Button(exp_frame, text="名前を付けて保存...", command=self.export_display_as).pack(fill=tk.X, pady=(6, 0))

    def _add_param_slider(self, parent: ttk.Frame, key: str, label: str, vmin: int, vmax: int, default: int) -> None:
        ...
        ttk.Scale(row, from_=vmin, to=vmax, orient=tk.HORIZONTAL, variable=var, command=on_change).pack(
